    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-md flex flex-col h-full p-0 gap-0 bg-background">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-background">
          ...
          {/* Valor Total */}
          <div className="bg-secondary rounded-[var(--radius)] p-6 text-white shadow-md text-center">
          ...
          {/* Lista de Itens */}
          <div className="bg-card rounded-[var(--radius)] border border-gray-200 p-4 shadow-sm">
            ...
            <div className="divide-y divide-gray-100">
                {order?.order_items.map((item: any, idx: number) => (
                  <div key={idx} className="py-3 flex justify-between items-center">
                      ...
                      <Badge className="bg-secondary">x{item.quantity}</Badge>
                  </div>
                ))}
            </div>
          </div>
