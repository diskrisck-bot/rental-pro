...
import { 
  ...
  Download, AlertTriangle, XCircle, Truck, Store, MapPin
} from 'lucide-react';
...
const OrderDetailsSheet = ({ orderId, open, onOpenChange, onStatusUpdate }: OrderDetailsSheetProps) => {
  ...
  const generatePDF = async (order: any, owner: OwnerProfile | null) => {
    ...
    // Parsing do Composite Field
    const rawMethod = order.delivery_method || 'pickup';
    const isDelivery = rawMethod.startsWith('delivery');
    const address = isDelivery ? rawMethod.split('|')[1] : null;
    ...
    if (isDelivery && address) {
        doc.text(`LOCAL DE ENTREGA: ${address}`, margin + 90, startBoxY + 20);
    } else {
    ...
    if (!isDelivery) {
        ...
        transporteText = "O LOCADOR responsabiliza-se pela entrega e retirada dos bens no endereço indicado pelo LOCATÁRIO. LOCAL DE ENTREGA/USO: " + (address || "Endereço não informado") + ". O LOCATÁRIO deve garantir que haja pessoa autorizada para receber e conferir os bens no horário agendado.";
    }
  ...
  const updateStatus = async (newStatus: string) => {
    ...
    try {
      ...
      if(error) throw error;
      showSuccess(newStatus === 'picked_up' ? "Equipamento retirado!" : "Equipamento devolvido!");
  ...
  const handleCancelOrder = async () => { if (window.confirm("Deseja cancelar este pedido?")) { await updateStatus('canceled'); onOpenChange(false); } };
  ...
  const getStatusBadge = () => {
      switch(status) {
          ...
          case 'picked_up': return <Badge className="bg-primary">Em Andamento</Badge>; 
      ...
      <SheetContent className="sm:max-w-md flex flex-col h-full p-0 gap-0 bg-background">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-background">
          ...
          {isDelivery && address && (
              <div className="bg-card rounded-[var(--radius)] border border-gray-200 p-4 shadow-custom">
                  <h4 className="text-xs font-bold text-gray-500 uppercase mb-2 flex items-center gap-2"><MapPin className="h-4 w-4"/> Local de Entrega/Uso</h4>
        ...
        <SheetFooter className="border-t border-gray-200 bg-card mt-auto w-full">
            <div className="p-6 flex flex-col gap-3 w-full">
                {status === 'signed' && (
                    <Button className="w-full h-14 bg-success hover:bg-success/90 text-white font-bold uppercase text-lg shadow-lg" onClick={() => updateStatus('picked_up')} disabled={updating}>
                        <Truck className="mr-2 h-6 w-6" /> CONFIRMAR RETIRADA
                    </Button>
                )}
                {status === 'reserved' && (
                    <Button className="w-full h-14 bg-primary hover:bg-primary/90 text-white font-bold uppercase text-lg shadow-lg" onClick={() => updateStatus('picked_up')} disabled={updating}>
                        <Truck className="mr-2 h-6 w-6" /> CONFIRMAR RETIRADA
                    </Button>
                )}
                {status === 'picked_up' && (
                    <Button className="w-full h-14 bg-success hover:bg-success/90 text-white font-bold uppercase text-lg shadow-lg" onClick={() => updateStatus('returned')} disabled={updating}>
                ...
                {!isFinalized && (
                    <Button variant="ghost" className="w-full text-destructive hover:text-destructive/90 hover:bg-destructive/10 font-bold uppercase text-xs mt-2" onClick={handleCancelOrder} disabled={updating}>
