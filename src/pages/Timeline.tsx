...
const Timeline = () => {
  ...
  // 2. Busca Pedidos Ativos (Assinados, Reservados ou Na Rua) para o intervalo
  const { data: orderItems, isLoading: loadingOrders } = useQuery({
    queryKey: ['timeline_orders', viewStartDay],
    queryFn: async () => {
      // Pega uma margem de segurança para buscar pedidos
      ...
      const { data } = await supabase
        ...
        `)
        .in('orders.status', ['signed', 'reserved', 'picked_up']) // STATUS ATIVOS INCLUSOS
        .lte('orders.start_date', rangeEnd) // Otimização: Só carrega o que sobrepõe a tela
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 h-screen flex flex-col">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div><h1 className="text-2xl md:text-3xl font-bold tracking-tight">Timeline</h1><p className="text-muted-foreground">Disponibilidade visual.</p></div>
      ...
      <div className="flex-1 bg-white border rounded-xl overflow-hidden relative">
        <div className="overflow-x-auto h-full">
          <div className="inline-block min-w-full">
            ...
            {/* Rows */}
            {products?.length === 0 ? <div className="p-10 text-center text-gray-400">Nenhum produto.</div> : 
               ...
               return (
                 <div key={product.id} className="flex border-b last:border-b-0 hover:bg-gray-50 transition-colors group">
                   <div className="w-64 p-4 border-r font-medium text-gray-700 bg-white sticky left-0 z-10 group-hover:bg-gray-50">
                     {product.name}
                     {product.type === 'bulk' && <span className="block text-xs text-blue-500 font-normal">Qtd Total: {product.total_quantity}</span>}
                   </div>
                   <div className="flex flex-1 relative h-16">
                     {days.map((day) => <div key={day.toString()} className="flex-1 min-w-[120px] border-r last:border-r-0 bg-grid-pattern opacity-10" />)}
                     ...
                     {allocations.map((item: any, index: number) => {
                       ...
                       return (
                         <Link 
                           key={order.id + index} to={`/orders?id=${order.id}`}
                           className={cn(`absolute top-2 h-12 rounded-md border shadow-sm flex items-center px-3 text-xs font-bold overflow-hidden whitespace-nowrap z-10 transition-all cursor-pointer hover:shadow-lg`, colorConfig.bg, colorConfig.border, colorConfig.text)}
                           style={{ left: `${position.left}px`, width: `${position.width}px` }}
                           title={`Cliente: ${order.customer_name}`}
                         >
